#!/usr/bin/env python3
"""Pliki 1.0 z napisem XpertLab (imprint.xpertlab) na produkcji (api.tiolibri.com), Ewa albo Bozena.
...
Uzycie: python3 generuj.py ewa|bozena
"""
import hashlib
import json
import pathlib
...
OUT = pathlib.Path.home() / "Downloads" / "xpertlab-2026-09-23"
...
# Sprawdzian: dopisek w metryce i kroj napisu w PDF, napis w EPUB.
...
tytul = subprocess.run(["unzip", "-p", str(epub), "EPUB/title.xhtml"], capture_output=True, text=True).stdout
print("PDF metryka:", "We współpracy z XpertLab" in txt, "| PDF kroj:", "Hanken" in fonts, "| EPUB napis:", "xlab-logo" in tytul)
