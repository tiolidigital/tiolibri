...
> **KOREKTA PO R2 (2026-08-07, `_review/R2-opus-response.md`).** Review Codexa wykazało, że trzy
> rekordy poniżej miały `werdykt=PASS` nieuprawniony treścią własnych pól — dwa z `C ⊄ E`
> (rozciągnięte `C` na wnioski o W2, których przebieg `--tylko-w1` nie wykonał) i jeden bez pola
> `E` w ogóle. Rekordy są **poprawione w miejscu, z tym znacznikiem**, a nie przepisane po cichu:
> liczby (3,44×, 215, 27) się nie zmieniły i nie były kwestionowane — zmieniło się to, co te
> rekordy o sobie twierdzą. Zawężone `C` odpowiada teraz dokładnie temu, co wykonano; wnioski
> wypchnięte z `C` są oznaczone jako inferencja z kontraktu w specu v0.4 (§`_media/`, §Krok 4).
